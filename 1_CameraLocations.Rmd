...
cameras <- read_csv("data/camerasites_2021_2022.csv") %>%
  ...
  select(camera,longitude, latitude, set_date, close_date, n_days, elevation_m)
```
...
cameras %>% 
  filter(camera != "SLC 201-Hushe-UNKNOWN") %>%
  write_csv(., "data/cameratraps.csv")

#save as shapefile for getting site boundary for covariates
cameras %>% 
  filter(camera != "SLC 201-Hushe-UNKNOWN") %>% #remove missing camera
